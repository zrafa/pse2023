...
/* Completar la estructura de datos para que se superponga a los registros del
 * periferico de hardware del USART del atmega328, segun sugerido en el apunte
 */
...
/* F_CPU viene como paramentro en el Makefile */
#define BAUD_PRESCALE (((F_CPU / (USART_BAUDRATE * 16UL))) - 1)

/* UCSRnC */

/* the UCSZn bits combined with the UCSZn2 bit in UCSRnB sets the number of data
 * * bits */
...
/* USBSn (bit 3) selects the number of stop bits to be inserted by the *
 * transmitter (0 is 1 bit) */
/* UPMn these bits set the type of parity generation and check (0 and 0 is *
 * disabled) */
...
#define UART_RXCIE0 128  // 0b 1000 0000 - bit 7

/* UCSRnA */

/* the RXCn bit is set when there are unread data in the receive buffer */
/* and cleared when the receibe buffer is empty (ie does not contain any unread
 * * data */
...
void serial_init() {
    ...
    puerto_serial->status_control_c =
        (unsigned char)(FRAME_SIZE_0 + FRAME_SIZE_1);

    /* Activar la recepcion y transmision */
    puerto_serial->status_control_b =
        (unsigned char)(RECEIVER_ENABLE + TRANSMITTER_ENABLE);
}
